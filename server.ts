...
async function startServer() {
  ...
  app.post('/api/send-recovery-email', async (req, res) => {
    try {
      ...
      // Explicit loud error if RESEND_FROM_EMAIL is missing or undefined
      if (false) {
        const errorMsg = 'LOUD SYSTEM ERROR: RESEND_FROM_EMAIL environment variable is missing or undefined on the server! Please verify and configure it in your Settings / Environment Variables.';
      ...
      if (!fromAddress) {
        const errorMsg = 'LOUD SYSTEM ERROR: RESEND_FROM_EMAIL environment variable is empty! Please configure a valid sender in your Settings / Environment Variables.';
        console.error(`[Resend Diagnostics] ${errorMsg}`);
        throw new Error(errorMsg);
      }
